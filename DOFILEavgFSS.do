...
bysort year: egen p25 = pctile(FSS_support), p(25)
by year: egen p75 = pctile(FSS_support), p(75)
by year: egen p50 = pctile(FSS_support), p(50)
...
sort year
by year: egen cp50=pctile(FSS_support), p(50), if cs==1
by year: egen ncp50=pctile(FSS_support), p(50), if cs==0
...
keep if anycs==1
keep id instnm year FSS_support cs
egen first_cs = min(year / (cs==1)), by(id)
gen t = (year-first_cs)

sort t
by t: egen taverage=pctile(FSS_support), p(50)
